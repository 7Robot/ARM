...
#define DEFAULT_OUTPUT_FORMAT	txt
...
{
#ifdef LONG_OPTIONS
	printf("Usage: %s [-i|--input=<bin|dec|hex|txt>] [-o|--option=<bin|dec|hex|txt>] [-h|--help]\n", cmd);
#else
	printf("Usage: %s [-i <bin|dec|hex|txt>] [-o <bin|dec|hex|txt>] [-h]\n", cmd);
#endif
}
...
void listener(can_t packet)
{
	if (can_pwrite(1, output_format, &packet) < 0) {
		perror("CAN_write");
		exit(1);
	}
}
