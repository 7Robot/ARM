...
int run_repeater_on(int listening_socket)
{
    ...
    /* Ignore SIGPIPE (function that should produce SIGPIPE return -1 and set
     *  errno to EPIPE) */
    install_sig_handler(SIGPIPE, 0, NULL, SIG_IGN);
						...
						for (int i = 0 ; i < socketc ; i++) {
							if (FD_ISSET(sockets[i], &targetfdset)) {
								if ((written = write(sockets[i], target_buffer[i], target[i])) < 0) {
									if (errno == EPIPE) {
										lerror(LOG_INFO, "write");
										close(sockets[i]);
										if (i != --socketc) {
											sockets[i] = sockets[socketc];
											memcpy(target_buffer[i], target_buffer[socket], BUF_SIZE);
											target[i] = target[socket];
										}
										lprintf(LOG_INFO, "%i connections [-1]", socketc);
									} else {
