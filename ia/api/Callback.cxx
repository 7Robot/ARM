...
void Callback::packet(struct libcan::can_t packet)
{
...
void Callback::microswitchEvent(int id, bool status)
{
...
void Callback::asservDone(int error)
{
    std::set<Mission*>::iterator it;
	for(it =  MissionHandler::missions.begin(); it != MissionHandler::missions.end(); ++it) {
		printf("\t[%d]\t%s::asserv\n", ((*it)->getState)(), ((*it)->getName)());
...
void Callback::sonarEvent(int id, bool edge, bool nearby, int distance)
{
    std::set<Mission*>::iterator it;
	for(it =  MissionHandler::missions.begin(); it != MissionHandler::missions.end(); ++it) {
		printf("\t[%d]\t%s::sonar\n", ((*it)->getState)(), ((*it)->getName)());
...
void Callback::odometryEvent(int x, int y, int theta)
{
    std::set<Mission*>::iterator it;
	for(it =  MissionHandler::missions.begin(); it != MissionHandler::missions.end(); ++it) {
